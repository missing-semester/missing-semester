...
1. Use a coding agent to replace the `-` [Markdown bullet markers](https://spec.commonmark.org/0.31.2/#bullet-list-marker) with `*` bullet markers in the [lecture notes for today](https://raw.githubusercontent.com/missing-semester/missing-semester/refs/heads/master/_2026/development-environment.md) (note that just replacing all the "-" characters in the file would be incorrect, as there are many uses of that character that are not bullet markers). Does it complete the task via direct file edits? What are the downsides and limitations of an agent editing the file directly to complete such a task? Figure out how to prompt the agent such that it doesn't complete the task via direct file edits. Hint: a command-line tool mentioned in the [first lecture](/2026/course-shell/) might come in handy. Compare the experience to using a regex, as covered in the [Code Quality](/2026/code-quality/) lecture.
