...
1. Use a coding agent to accomplish the same goal as in the exercise above. Does it complete the tasks via direct file edits? What are the downsides and limitations of an agent editing the file directly to complete such a task? Figure out how to prompt the agent such that it doesn't complete the task via direct file edits. Hint: a command-line tool mentioned in the [first lecture](/2026/course-shell/) might come in handy.
